...
Options:
  ...
  --git          Configure git user.{name,email}:
                   - auto: set user.name if unset (from $USER); do not set email
                   - ask : interactively prompt (default)
                   - skip: do nothing
...
maybe_configure_git() {
  echo_heading "Git identity (optional)"
  case "${CONFIG_GIT}" in
  skip)
  ...
  auto)
    ...
    if [ -z "${current_name}" ]; then
      local guess_name="${USER}"
      if [ -n "${guess_name}" ]; then
        git config --global user.name "${guess_name}"
        ok "Set git user.name to \"${guess_name}\""
      fi
    else
      info "git user.name already set to \"${current_name}\""
    fi
    info "Not setting user.email in auto mode."
    ;;
  ask | *)
    read -r -p "Set git user.name now? [y/N]: " yn
    if [ "${yn}" = "y" ] || [ "${yn}" = "Y" ]; then
      read -r -p "user.name: " gname
      if [ -n "${gname}" ]; then
        git config --global user.name "${gname}"
        ok "user.name set."
      fi
    fi
    read -r -p "Set git user.email now? [y/N]: " ye
    if [ "${ye}" = "y" ] || [ "${ye}" = "Y" ]; then
...
- If GitHub says "Permission denied (publickey)":
  ...
  4) Verify config in ~/.ssh/config (example below)
...
- GIT_SSH_COMMAND='ssh -v' git ls-remote git@github.com:your-user/your-repo.git HEAD
